...
public:
    ...
    RIGHT_VECTOR b(Columns()); // used for piece of the identity matrix
...
{output_stream<<"[";for(int i=1;i<=A.Rows();i++){for(int j=1;j<=A.Columns();j++){output_stream<<std::setw(12)<<A(i,j);if(j<A.Columns()) output_stream<<" ";}if(i<A.Rows()) output_stream<<"; ";}output_stream<<"]";return output_stream;}

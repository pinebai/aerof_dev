...
namespace ARRAYS_COMPUTATIONS
{
    ...
    {const T_ARRAY& self=a.Derived();T result=abs(self(ID(1)));ID m=self.Size();for(ID i(2);i<=m;i++) result=PhysBAM::max(result,abs(self(i)));return result;}

    template<class T,class T_ARRAY,class ID>
    T Maxmag(const ARRAY_BASE<T,T_ARRAY,ID>& a)
    {const T_ARRAY& self=a.Derived();T result=self(ID(1));ID m=self.Size();for(ID i(2);i<=m;i++) result=PhysBAM::maxmag(result,self(i));return result;}
    ...
    {const T_ARRAY& self=a.Derived();T result=abs(self(ID(1)));ID m=self.Size();for(ID i(2);i<=m;i++) result=T::Componentwise_Max(result,abs(self(i)));return result;}
}
}
#endif

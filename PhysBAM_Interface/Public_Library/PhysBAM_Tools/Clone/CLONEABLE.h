...
public:
    ...
    {return static_cast<T_DERIVED*>(Clone_Implementation());}

    T_DERIVED* Clone_Default() const // creates a default constructed copy of the same type
    {return static_cast<T_DERIVED*>(Clone_Default_Implementation());}
};
//#####################################################################
// Class CLONEABLE
//#####################################################################
...
private:
    ...
    clone->Clone_Helper(static_cast<const T_DERIVED&>(*this));
    return clone;}

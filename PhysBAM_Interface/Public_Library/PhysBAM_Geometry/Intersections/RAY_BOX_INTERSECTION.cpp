...
#include <PhysBAM_Geometry/Intersections/RAY_PLANE_INTERSECTION.h>
#include <PhysBAM_Geometry/Intersections/RAY_SEGMENT_2D_INTERSECTION.h>
namespace PhysBAM{
namespace INTERSECTION{
//#####################################################################
// Function Intersects
...
template<class T> bool Lazy_Intersects(RAY<VECTOR<T,3> >& ray,const RANGE<VECTOR<T,3> >& box,const T thickness_over_two)
{
    ...
    if(t_min>=0){if(ray.semi_infinite || t_min<t_max){ray.t_max=t_min;ray.semi_infinite=false;ray.intersection_location=RAY<VECTOR<T,3> >::INTERIOR_POINT;return true;}}
    else if(t_max>=0 && (ray.semi_infinite || t_max<t_max)){ray.t_max=t_max;ray.semi_infinite=false;ray.intersection_location=RAY<VECTOR<T,3> >::INTERIOR_POINT;return true;}
    return false;
}
//#####################################################################

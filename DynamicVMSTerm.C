...
DynamicVMSTerm::DynamicVMSTerm(IoData& iod, VarFcn *vf): NavierStokesTerm(iod, vf)
{
  ...
  csmax = iod.eqs.tc.les.dvms.clip.cs_max;
  ptmin = iod.eqs.tc.les.dvms.clip.pt_min;
  ptmax = iod.eqs.tc.les.dvms.clip.pt_max; 
...
void DynamicVMSTerm::compute(double Cs[4], double Pt[4], double tetVol, double dp1dxj[4][3], double *VBar[4],
			     double *V[4], double *r, SVec<double,3> &X, int nodeNum[4], bool clip)

{

  double (*R)[5] = reinterpret_cast<double (*)[5]>(r);

  // Compute small scale Reynolds stress tensor and small scale energy gradient //
...
double DynamicVMSTerm::computeEddyViscosity(double rhoBar, double duidxj[3][3])
{
  ...
  return (rhoBar)*csprime*sqrt2Sprime2;

}

//------------------------------------------------------------------------

double DynamicVMSTerm::computeFilterWidth(double tetVol, SVec<double,3> &X, int nodeNum[4])
...
void DynamicVMSTerm::computeVelocityGradient(double dp1dxj[4][3],
					     double u[4][3],
					     double dudxj[3][3])
...
void DynamicVMSTerm::computeStressTensor(double mu, double dudxj[3][3],
				         double tij[3][3])
...
void DynamicVMSTerm::getPrimeValues(double *V[4], double *VBar[4],
...
void DynamicVMSTerm::computeSmallEnergyGradient(double dp1dxj[4][3],
					        double eprime[4],
					        double dePrimedxj[3])

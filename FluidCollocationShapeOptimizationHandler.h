#ifndef _FLUID_COLLOCATION_SHAPE_OPTIMIZATION_HANDLER_H_
#define _FLUID_COLLOCATION_SHAPE_OPTIMIZATION_HANDLER_H_
...
template<int dim> class ImplicitCollocationTsDesc;

//------------------------------------------------------------------------------
template<int dim>
class FluidCollocationShapeOptimizationHandler : public ImplicitCollocationTsDesc<dim> {
...
public:
  ...
  /// \note The pointer 'dom' is passed to ImplicitCollocationTsDesc
  /// and stored in the member variable domain.
  /// It seems redundant (UH - 08/10)
  FluidCollocationShapeOptimizationHandler
  (
    IoData &ioData,
    GeoSource &geoSource,
    Domain *dom//,
...
#ifdef TEMPLATE_FIX
#include <FluidCollocationShapeOptimizationHandler.C>
#endif

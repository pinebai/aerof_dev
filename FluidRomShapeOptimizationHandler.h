#ifndef _FLUID_ROM_SHAPE_OPTIMIZATION_HANDLER_H_
#define _FLUID_ROM_SHAPE_OPTIMIZATION_HANDLER_H_
...
template<int dim> class ImplicitPGTsDesc;
...
//------------------------------------------------------------------------------
template<int dim>
class FluidRomShapeOptimizationHandler : public ImplicitPGTsDesc<dim> {

private:

  Domain *domain;
//  TsSolver<ImplicitRomTsDesc<dim> > *tsSolver;
...
public:
  ...
  /// \note The pointer 'dom' is passed to ImplicitRomTsDesc
  /// and stored in the member variable domain.
  /// It seems redundant (UH - 08/10)
  FluidRomShapeOptimizationHandler
  (
    IoData &ioData,
    GeoSource &geoSource,
    Domain *dom//,
...
#ifdef TEMPLATE_FIX
#include <FluidRomShapeOptimizationHandler.C>
#endif

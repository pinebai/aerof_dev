#include <FluidCollocationShapeOptimizationHandler.h>
...
template<int dim>
FluidCollocationShapeOptimizationHandler<dim>::FluidCollocationShapeOptimizationHandler
(
  IoData &ioData,
  GeoSource &geoSource,
  Domain *dom//,
) :
ImplicitCollocationTsDesc<dim>(ioData, geoSource, dom),
domain(dom),
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoRestartBcFluxs(IoData &ioData)
{
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoGetEfforts(IoData &ioData, 
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoGetDerivativeOfEffortsFiniteDifference(IoData &ioData,
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoGetDerivativeOfEffortsAnalytical(IoData &ioData,
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoGetDerivativeOfLoadFiniteDifference(IoData &ioData, DistSVec<double,3> &X, DistSVec<double,3> &dX, DistVec<double> &A,
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoGetDerivativeOfLoadAnalytical(IoData &ioData, DistSVec<double,3> &X, DistSVec<double,3> &dX, 
                                             DistSVec<double,dim> &U, DistSVec<double,dim> &dU, DistSVec<double,3> &load, DistSVec<double,3> &dLoad)
{
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoSemiAnalytical
(
  IoData &ioData, 
  DistSVec<double,3> &X,
  DistVec<double> &A,
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoAnalytical
...
//void FluidCollocationShapeOptimizationHandler<dim>::computeAJ(int it, DistSVec<double, dim> &Q)  {
//
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoSetUpLinearSolver(IoData &ioData, DistSVec<double,3> &X, DistVec<double> &A, DistSVec<double,dim> &U)
{
  ...
  // Set up reduced linear system (for COLLOCATION: Psi = dFdw*Phi)
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoPrintTextOnScreen(const char *Text)
{
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoOutput1D(const char *fileName, DistVec<double> &V)
{
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoOutput3D(const char *fileName, DistSVec<double,3> &V)
{
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoOutputDimD(const char *fileName, DistSVec<double,dim> &V)
{
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoInitialize(IoData &ioData, DistSVec<double,dim> &U)
{
...
template<int dim>
int FluidCollocationShapeOptimizationHandler<dim>::fsoHandler(IoData &ioData, DistSVec<double,dim> &U)
{
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoComputeDerivativesOfFluxAndSolution(IoData &ioData, DistSVec<double,3> &X, DistVec<double> &A, DistSVec<double,dim> &U)
{
  ...
  if (ioData.sa.lsSolver == SensitivityAnalysis::NORMAL_EQUATIONS) {

    // Form the normal equations
    double t0 = this->timer->getTime();
    (this->rom->restrictMapping())->restriction(dFdS, *this->ResRestrict);
    this->projectVector(*this->AJRestrict, *this->ResRestrict, this->From);    
    this->rhs = -1.0 * this->From;
    this->timer->addLinearSystemFormTime(t0);
  
    // Solve the normal equations
    t0 = this->timer->getTime();
    this->solveLinearSystem(0, this->rhs, dYdS);
    this->timer->addLinearSystemSolveTime(t0);
  
  } else {
    this->com->fprintf(stderr, "Only normal equations supported for collocation sensitivity analysis\n");
    exit(-1);
  }
...
template<int dim>
void FluidCollocationShapeOptimizationHandler<dim>::fsoComputeSensitivities(IoData &ioData, const char *mesage, const char *fileName, DistSVec<double,3> &X, DistSVec<double,dim> &U)
{

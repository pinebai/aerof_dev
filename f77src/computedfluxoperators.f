      SUBROUTINE COMPUTEDFLUXOPERATORS(type,gamma,gam,pstiff,dpstiff,
     &                      enormal,evitno,Ugr,
     &                      Ug,Udr,Ud,phi,
     &                      dphidenormal, dphidevitno, dphidug, 
     &                      dphidud)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      REAL*8 dphidug(7,7), dphidud(7,7)
      REAL*8 dphidenormal(3,7), dphidevitno(1,7)
      REAL*8 Ugr(*), Udr(*), energ, enerd
      REAL*8 dphidvp1(1,7), dphidvp4(1,7), dphidvp5(1,7)
      REAL*8 H, vitno, updir, gamma 
      REAL*8 VdotN, rnorm, invnorm
      REAL*8 flur1, flur2, flur3, flur4, flur5
      REAL*8 dvp1denormal(3,1), dvp1devitno(1,1), dvp1dug(7,1)
      REAL*8 dvp1dud(7,1) 
      REAL*8 dvp4denormal(3,1), dvp4devitno(1,1), dvp4dug(7,1)
      REAL*8 dvp4dud(7,1) 
      REAL*8 dvp5denormal(3,1), dvp5devitno(1,1), dvp5dug(7,1) 
      REAL*8 dvp5dud(7,1)
...
c        dvp1 = 0.0d0
        dvp1denormal(1,1) = 0.0d0
        dvp1denormal(2,1) = 0.0d0
        dvp1denormal(3,1) = 0.0d0
        dvp1devitno(1,1) = 0.0d0
        dvp1dug(1,1) = 0.0d0
        dvp1dug(2,1) = 0.0d0
        dvp1dug(3,1) = 0.0d0
        dvp1dug(4,1) = 0.0d0
        ...
        dvp1denormal(1,1) = cf(271)
        dvp1denormal(2,1) = cf(272)
        dvp1denormal(3,1) = cf(273)
        dvp1devitno(1,1) = -cf(274)
        dvp1dug(1,1) = cf(275)
        dvp1dug(2,1) = cf(276)
        dvp1dug(3,1) = cf(277)
        dvp1dug(4,1) = cf(278)
...
c        dvp4 = 0.0d0
        dvp4denormal(1,1) = 0.0d0
        dvp4denormal(2,1) = 0.0d0 
        dvp4denormal(3,1) = 0.0d0
        dvp4devitno(1,1) = 0.0d0
        dvp4dug(1,1) = 0.0d0
        dvp4dug(2,1) = 0.0d0
        dvp4dug(3,1) = 0.0d0
        dvp4dug(4,1) = 0.0d0
     ...
     &    - vitno)/ABS((VdotN + cr) - vitno)
        ...
        dvp4denormal(1,1) = cf(284)
        dvp4denormal(2,1) = cf(285)
        dvp4denormal(3,1) = cf(286)
        dvp4devitno(1,1) = cf(298)
        dvp4dug(1,1) = cf(288)
        dvp4dug(2,1) = cf(289)
        dvp4dug(3,1) = cf(290)
        dvp4dug(4,1) = cf(291)
...
c     &      + cf(298)*devitno
      ...
      if (ABS((VdotN - cr) - vitno) .eq. 0.0d0) then
c        dvp5 = 0.0d0
        dvp5denormal(1,1)=0.0d0
        dvp5denormal(2,1)=0.0d0
        dvp5denormal(3,1)=0.0d0
        dvp5dug(1,1)=0.0d0
        dvp5dug(2,1)=0.0d0
        dvp5dug(3,1)=0.0d0
        dvp5dug(4,1)=0.0d0
     ...
     &            - vitno)/ABS((VdotN - cr) - vitno)
        ...
        dvp5denormal(1,1)=cf(300)
        dvp5denormal(2,1)=cf(301)
        dvp5denormal(3,1)=cf(302)
        dvp5dug(1,1)=cf(304)
        dvp5dug(2,1)=cf(305)
        dvp5dug(3,1)=cf(306)
        dvp5dug(4,1)=cf(307)
...
c
      ...
      dphidvp4(1,1) = cf(803)
      dphidvp5(1,1) = cf(804)
      dphidenormal(1,1) = cf(805)
      dphidenormal(2,1) = cf(806)
      dphidenormal(3,1) = cf(807)
      dphidevitno(1,1) = cf(808)
      dphidug(1,1) = cf(810)+cf(825)
      dphidug(2,1) = cf(811)+cf(826)
      dphidug(3,1) = cf(812)+cf(827)
      dphidug(4,1) = cf(813)+cf(828)
      dphidug(5,1) = cf(814)+cf(829)
      dphidug(6,1) = 0.0d0
      dphidug(7,1) = 0.0d0
      dphidud(1,1) = cf(815)+cf(820)
      dphidud(2,1) = cf(816)+cf(821)
      dphidud(3,1) = cf(817)+cf(822)
      dphidud(4,1) = cf(818)+cf(823)
      dphidud(5,1) = cf(819)+cf(824)
      dphidud(6,1) = 0.0d0
      dphidud(7,1) = 0.0d0
      DO 10, I = 1,3
        dphidenormal(I,1) = dphidenormal(I,1) 
     &                    + dphidvp1(1,1)*dvp1denormal(I,1) 
     &                    + dphidvp4(1,1)*dvp4denormal(I,1) 
     &                    + dphidvp5(1,1)*dvp5denormal(I,1) 
   10 CONTINUE
      DO 20, I = 1,7
        dphidug(I,1) = dphidug(I,1) 
     &               + dphidvp1(1,1)*dvp1dug(I,1) 
     &               + dphidvp4(1,1)*dvp4dug(I,1) 
     &               + dphidvp5(1,1)*dvp5dug(I,1) 
        dphidud(I,1) = dphidud(I,1) 
     &               + dphidvp1(1,1)*dvp1dud(I,1) 
     &               + dphidvp4(1,1)*dvp4dud(I,1) 
     &               + dphidvp5(1,1)*dvp5dud(I,1) 
   20 CONTINUE
      dphidevitno(1,1) = dphidevitno(1,1) 
     &                 + dphidvp1(1,1)*dvp1devitno(1,1)
     &                 + dphidvp4(1,1)*dvp4devitno(1,1)
     &                 + dphidvp5(1,1)*dvp5devitno(1,1)
c      dphidugr(1,1) = cf(825)
c      dphidugr(2,1) = cf(826)
c      dphidugr(3,1) = cf(827)
c      dphidugr(4,1) = cf(828)
c      dphidugr(5,1) = cf(829)
c      dphidugr(6,1) = 0.0d0
c      dphidugr(7,1) = 0.0d0
c      dphidudr(1,1) = cf(820)
c      dphidudr(2,1) = cf(821)
c      dphidudr(3,1) = cf(822)
c      dphidudr(4,1) = cf(823)
c      dphidudr(5,1) = cf(824)
c      dphidudr(6,1) = 0.0d0
c      dphidudr(7,1) = 0.0d0
c     dphi(1) = cf(802)*dvp1
c     &         + cf(803)*dvp4
c     &         + cf(804)*dvp5
c     &         + cf(805)*denormal(1)
c     &         + cf(806)*denormal(2)
...
c     &         + cf(829)*dUgr(5)
      ...
      cf(830)=0.5*rnorm
      cf(831)=phi(2)*0.5
      dphidvp1(1,2)=-cf(830)*cf(666)
      dphidvp4(1,2)=-cf(830)*cf(678)
      dphidvp5(1,2)=-cf(830)*cf(679)
      dphidenormal(1,2)=(cf(830)*cf(680)+cf(831)*cf(2)) 
      dphidenormal(2,2)=(cf(831)*cf(3)+cf(830)*cf(681))
      dphidenormal(3,2)=(cf(831)*cf(4)+cf(830)*cf(682))
      dphidevitno(1,2)=cf(830)*cf(97)
      dphidug(1,2)=cf(830)*cf(683)-cf(830)*cf(673)
      dphidug(2,2)=cf(830)*cf(684)-cf(830)*cf(674)
      dphidug(3,2)=cf(830)*cf(685)-cf(830)*cf(675)
      dphidug(4,2)=cf(830)*cf(686)-cf(830)*cf(676)
      dphidug(5,2)=cf(830)*cf(687)-cf(830)*cf(677)
      dphidug(6,2)=0.0d0
      dphidug(7,2)=0.0d0
      dphidud(1,2)=cf(830)*cf(688)-cf(830)*cf(668)
      dphidud(2,2)=cf(830)*cf(689)-cf(830)*cf(669)
      dphidud(3,2)=cf(830)*cf(690)-cf(830)*cf(670)
      dphidud(4,2)=cf(830)*cf(691)-cf(830)*cf(671)
      dphidud(5,2)=cf(830)*cf(692)-cf(830)*cf(672)
      dphidud(6,2)=0.0d0
      dphidud(7,2)=0.0d0
      DO 30, I = 1,3
        dphidenormal(I,2) = dphidenormal(I,2) 
     &                    + dphidvp1(1,2)*dvp1denormal(I,1) 
     &                    + dphidvp4(1,2)*dvp4denormal(I,1) 
     &                    + dphidvp5(1,2)*dvp5denormal(I,1) 
   30 CONTINUE
      DO 40, I = 1,7
        dphidug(I,2) = dphidug(I,2) 
     &               + dphidvp1(1,2)*dvp1dug(I,1) 
     &               + dphidvp4(1,2)*dvp4dug(I,1) 
     &               + dphidvp5(1,2)*dvp5dug(I,1) 
        dphidud(I,2) = dphidud(I,2) 
     &               + dphidvp1(1,2)*dvp1dud(I,1) 
     &               + dphidvp4(1,2)*dvp4dud(I,1) 
     &               + dphidvp5(1,2)*dvp5dud(I,1) 
   40 CONTINUE
      dphidevitno(1,2) = dphidevitno(1,2) 
     &                 + dphidvp1(1,2)*dvp1devitno(1,1)
     &                 + dphidvp4(1,2)*dvp4devitno(1,1)
     &                 + dphidvp5(1,2)*dvp5devitno(1,1)
c      dphidudr(1,2)=-cf(830)*cf(668)
c      dphidudr(2,2)=-cf(830)*cf(669)
c      dphidudr(3,2)=-cf(830)*cf(670)
c      dphidudr(4,2)=-cf(830)*cf(671)
c      dphidudr(5,2)=-cf(830)*cf(672)
c      dphidudr(6,2)=0.0d0
c      dphidudr(7,2)=0.0d0
c      dphidugr(1,2)=-cf(830)*cf(673)
c      dphidugr(2,2)=-cf(830)*cf(674)
c      dphidugr(3,2)=-cf(830)*cf(675)
c      dphidugr(4,2)=-cf(830)*cf(676)
c      dphidugr(5,2)=-cf(830)*cf(677)
c      dphidugr(6,2)=0.0d0
c      dphidugr(7,2)=0.0d0
c      dphi(2) = -cf(830)*cf(666)*dvp1
c     &         - cf(830)*cf(678)*dvp4
c     &         - cf(830)*cf(679)*dvp5
c     &         + (cf(830)*cf(680)+cf(831)*cf(2))*denormal(1)
c     &         + (cf(831)*cf(3)+cf(830)*cf(681))*denormal(2)
c     &         + (cf(831)*cf(4)+cf(830)*cf(682))*denormal(3)
c     %         + cf(830)*cf(97)*devitno
c     &         - cf(830)*cf(667)*dpstiff
c     %         + cf(830)*cf(683)*dUg(1)
c     %         + cf(830)*cf(684)*dUg(2)
c     %         + cf(830)*cf(685)*dUg(3)
c     %         + cf(830)*cf(686)*dUg(4)
c     %         + cf(830)*cf(687)*dUg(5)
c     &         + cf(830)*cf(688)*dUd(1)
c     &         + cf(830)*cf(689)*dUd(2)
c     &         + cf(830)*cf(690)*dUd(3)
c     &         + cf(830)*cf(691)*dUd(4)
c     &         + cf(830)*cf(692)*dUd(5)
c     &         - cf(830)*cf(668)*dUdr(1)
c     &         - cf(830)*cf(669)*dUdr(2)
c     &         - cf(830)*cf(670)*dUdr(3)
c     &         - cf(830)*cf(671)*dUdr(4)
c     &         - cf(830)*cf(672)*dUdr(5)
c     &         - cf(830)*cf(673)*dUgr(1)
c     &         - cf(830)*cf(674)*dUgr(2)
c     &         - cf(830)*cf(675)*dUgr(3)
c     &         - cf(830)*cf(676)*dUgr(4)
c     &         - cf(830)*cf(677)*dUgr(5)
      ...
      dphidvp1(1,3)=-0.5*rnorm*cf(706)
      dphidvp4(1,3)=- 0.5*rnorm*(cf(697)*cf(521)+cf(701)*cf(521))
      dphidvp5(1,3)=0.5*rnorm*(cf(697)*cf(580)-cf(701)*cf(580))
      dphidenormal(1,3)=cf(831)
      dphidenormal(2,3)=cf(832)
      dphidenormal(3,3)=cf(833)
      dphidevitno(1,3)=0.5*rnorm*cf(106)
      dphidug(1,3)=0.5*rnorm*(cf(107)-cf(708))-0.5*rnorm*cf(723)
      dphidug(2,3)=0.5*rnorm*(cf(108)-cf(709))-0.5*rnorm*cf(724)
      dphidug(3,3)=0.5*rnorm*(cf(109)-cf(710))-0.5*rnorm*cf(725)
      dphidug(4,3)=0.5*rnorm*(cf(110)-cf(711))-0.5*rnorm*cf(726)
      dphidug(5,3)=0.5*rnorm*(normal(2)-cf(712))-0.5*rnorm*cf(727)
      dphidug(6,3)=0.0d0
      dphidug(7,3)=0.0d0
      dphidud(1,3)=0.5*rnorm*(cf(111)-cf(713))-0.5*rnorm*cf(718)
      dphidud(2,3)=0.5*rnorm*(cf(112)-cf(714))-0.5*rnorm*cf(719)
      dphidud(3,3)=0.5*rnorm*(cf(113)-cf(715))-0.5*rnorm*cf(720)
      dphidud(4,3)=0.5*rnorm*(cf(114)-cf(716))-0.5*rnorm*cf(721)
      dphidud(5,3)=0.5*rnorm*(normal(2)-cf(717))-0.5*rnorm*cf(722)
      dphidud(6,3)=0.0d0
      dphidud(7,3)=0.0d0
      DO 50, I = 1,3
        dphidenormal(I,3) = dphidenormal(I,3) 
     &                    + dphidvp1(1,3)*dvp1denormal(I,1) 
     &                    + dphidvp4(1,3)*dvp4denormal(I,1) 
     &                    + dphidvp5(1,3)*dvp5denormal(I,1) 
   50 CONTINUE
      DO 60, I = 1,7
        dphidug(I,3) = dphidug(I,3) 
     &               + dphidvp1(1,3)*dvp1dug(I,1) 
     &               + dphidvp4(1,3)*dvp4dug(I,1) 
     &               + dphidvp5(1,3)*dvp5dug(I,1) 
        dphidud(I,3) = dphidud(I,3) 
     &               + dphidvp1(1,3)*dvp1dud(I,1)
     &               + dphidvp4(1,3)*dvp4dud(I,1) 
     &               + dphidvp5(1,3)*dvp5dud(I,1) 
   60 CONTINUE
      dphidevitno(1,3) = dphidevitno(1,3) 
     &                 + dphidvp1(1,3)*dvp1devitno(1,1)
     &                 + dphidvp4(1,3)*dvp4devitno(1,1)
     &                 + dphidvp5(1,3)*dvp5devitno(1,1)
c      dphidudr(1,3)=-0.5*rnorm*cf(718)
c      dphidudr(2,3)=-0.5*rnorm*cf(719)
c      dphidudr(3,3)=-0.5*rnorm*cf(720)
c      dphidudr(4,3)=-0.5*rnorm*cf(721)
c      dphidudr(5,3)=-0.5*rnorm*cf(722)
c      dphidudr(6,3)=0.0d0
c      dphidudr(7,3)=0.0d0
c      dphidugr(1,3)=-0.5*rnorm*cf(723)
c      dphidugr(2,3)=-0.5*rnorm*cf(724)
c      dphidugr(3,3)=-0.5*rnorm*cf(725)
c      dphidugr(4,3)=-0.5*rnorm*cf(726)
c      dphidugr(5,3)=-0.5*rnorm*cf(727)
c      dphidugr(6,3)=0.0d0
c      dphidugr(7,3)=0.0d0
c      dphi(3) =-0.5*rnorm*cf(706)*dvp1
c     &         - 0.5*rnorm*(cf(697)*cf(521)+cf(701)*cf(521))*dvp4
c     &         + 0.5*rnorm*(cf(697)*cf(580)-cf(701)*cf(580))*dvp5
c     &         + cf(831)*denormal(1)
c     &         + cf(832)*denormal(2)
c     &         + cf(833)*denormal(3)
c     &         + 0.5*rnorm*cf(106)*devitno
c     &         - 0.5*rnorm*cf(707)*dpstiff
c     &         + 0.5*rnorm*(cf(107)-cf(708))*dUg(1)
c     &         + 0.5*rnorm*(cf(108)-cf(709))*dUg(2)
c     &         + 0.5*rnorm*(cf(109)-cf(710))*dUg(3)
c     &         + 0.5*rnorm*(cf(110)-cf(711))*dUg(4)
c     &         + 0.5*rnorm*(normal(2)-cf(712))*dUg(5)
c     &         + 0.5*rnorm*(cf(111)-cf(713))*dUd(1)
c     &         + 0.5*rnorm*(cf(112)-cf(714))*dUd(2)
c     &         + 0.5*rnorm*(cf(113)-cf(715))*dUd(3)
c     &         + 0.5*rnorm*(cf(114)-cf(716))*dUd(4)
c     &         + 0.5*rnorm*(normal(2)-cf(717))*dUd(5)
c     &         - 0.5*rnorm*cf(718)*dUdr(1)
c     &         - 0.5*rnorm*cf(719)*dUdr(2)
c     &         - 0.5*rnorm*cf(720)*dUdr(3)
c     &         - 0.5*rnorm*cf(721)*dUdr(4)
c     &         - 0.5*rnorm*cf(722)*dUdr(5)
c     &         - 0.5*rnorm*cf(723)*dUgr(1)
c     &         - 0.5*rnorm*cf(724)*dUgr(2)
c     &         - 0.5*rnorm*cf(725)*dUgr(3)
c     &         - 0.5*rnorm*cf(726)*dUgr(4)
c     &         - 0.5*rnorm*cf(727)*dUgr(5)
      phi(3) = phi(3)*0.5*rnorm
      dphidvp1(1,4)= -0.5*rnorm*cf(761)
      dphidvp4(1,4)=0.5*rnorm*cf(762)
      dphidvp5(1,4)=0.5*rnorm*cf(763)
      dphidenormal(1,4)=(0.5*rnorm*cf(764)+phi(4)*0.5*cf(2))
      dphidenormal(2,4)=(0.5*rnorm*cf(765)+phi(4)*0.5*cf(3))
      dphidenormal(3,4)=(0.5*rnorm*cf(766)+phi(4)*0.5*cf(4))
      dphidevitno(1,4)=0.5*rnorm*cf(118)
      dphidug(1,4)=0.5*rnorm*(cf(60)-cf(737))-0.5*rnorm*cf(753) 
      dphidug(2,4)=0.5*rnorm*(cf(61)-cf(738))-0.5*rnorm*cf(754) 
      dphidug(3,4)=0.5*rnorm*(cf(62)-cf(739))-0.5*rnorm*cf(755) 
      dphidug(4,4)=0.5*rnorm*(cf(63)-cf(740))-0.5*rnorm*cf(756) 
      dphidug(5,4)=0.5*rnorm*(normal(3)-cf(741))-0.5*rnorm*cf(757)
      dphidug(6,4)=0.0d0
      dphidug(7,4)=0.0d0
      dphidud(1,4)=0.5*rnorm*(cf(119)-cf(742))-0.5*rnorm*cf(748)
      dphidud(2,4)=0.5*rnorm*(cf(120)-cf(743))-0.5*rnorm*cf(749)
      dphidud(3,4)=0.5*rnorm*(cf(121)-cf(744))-0.5*rnorm*cf(750)
      dphidud(4,4)=0.5*rnorm*(cf(122)-cf(745))-0.5*rnorm*cf(751)
      dphidud(5,4)=0.5*rnorm*(normal(3)-cf(746))-0.5*rnorm*cf(752)
      dphidud(6,4)=0.0d0
      dphidud(7,4)=0.0d0
      DO 70, I = 1,3
        dphidenormal(I,4) = dphidenormal(I,4) 
     &                    + dphidvp1(1,4)*dvp1denormal(I,1) 
     &                    + dphidvp4(1,4)*dvp4denormal(I,1) 
     &                    + dphidvp5(1,4)*dvp5denormal(I,1) 
   70 CONTINUE
      DO 80, I = 1,7
        dphidug(I,4) = dphidug(I,4) 
     &               + dphidvp1(1,4)*dvp1dug(I,1) 
     &               + dphidvp4(1,4)*dvp4dug(I,1) 
     &               + dphidvp5(1,4)*dvp5dug(I,1) 
        dphidud(I,4) = dphidud(I,4) 
     &               + dphidvp1(1,4)*dvp1dud(I,1) 
     &               + dphidvp4(1,4)*dvp4dud(I,1) 
     &               + dphidvp5(1,4)*dvp5dud(I,1) 
   80 CONTINUE
      dphidevitno(1,4) = dphidevitno(1,4) 
     &                 + dphidvp1(1,4)*dvp1devitno(1,1)
     &                 + dphidvp4(1,4)*dvp4devitno(1,1)
     &                 + dphidvp5(1,4)*dvp5devitno(1,1)
c      dphidudr(1,4)=-0.5*rnorm*cf(748)
c      dphidudr(2,4)=-0.5*rnorm*cf(749)
c      dphidudr(3,4)=-0.5*rnorm*cf(750)
c      dphidudr(4,4)=-0.5*rnorm*cf(751)
c      dphidudr(5,4)=-0.5*rnorm*cf(752)
c      dphidudr(6,4)=0.0d0
c      dphidudr(7,4)=0.0d0
c      dphidugr(1,4)=-0.5*rnorm*cf(753)
c      dphidugr(2,4)=-0.5*rnorm*cf(754)
c      dphidugr(3,4)=-0.5*rnorm*cf(755)
c      dphidugr(4,4)=-0.5*rnorm*cf(756)
c      dphidugr(5,4)=-0.5*rnorm*cf(757)
c      dphidugr(6,4)=0.0d0
c      dphidugr(7,4)=0.0d0
c      dphi(4) = -0.5*rnorm*cf(761)*dvp1
c     &        + 0.5*rnorm*cf(762)*dvp4
c     &         + 0.5*rnorm*cf(763)*dvp5
c     &      + (0.5*rnorm*cf(764)+phi(4)*0.5*cf(2))*denormal(1)
c     &      + (0.5*rnorm*cf(765)+phi(4)*0.5*cf(3))*denormal(2)
c     &      + (0.5*rnorm*cf(766)+phi(4)*0.5*cf(4))*denormal(3)
c     &         + 0.5*rnorm*cf(118)*devitno
c     &         - 0.5*rnorm*cf(747)*dpstiff
c     &         + 0.5*rnorm*(cf(60)-cf(737))*dUg(1)
c     &         + 0.5*rnorm*(cf(61)-cf(738))*dUg(2)
c     &         + 0.5*rnorm*(cf(62)-cf(739))*dUg(3)
c     &         + 0.5*rnorm*(cf(63)-cf(740))*dUg(4)
c     &         + 0.5*rnorm*(normal(3)-cf(741))*dUg(5)
c     &         + 0.5*rnorm*(cf(119)-cf(742))*dUd(1)
c     &         + 0.5*rnorm*(cf(120)-cf(743))*dUd(2)
c     &         + 0.5*rnorm*(cf(121)-cf(744))*dUd(3)
c     &         + 0.5*rnorm*(cf(122)-cf(745))*dUd(4)
c     &         + 0.5*rnorm*(normal(3)-cf(746))*dUd(5)
c     &         - 0.5*rnorm*cf(748)*dUdr(1)
c     &         - 0.5*rnorm*cf(749)*dUdr(2)
c     &      - 0.5*rnorm*cf(750)*dUdr(3)
c     &      - 0.5*rnorm*cf(751)*dUdr(4)
c     &      - 0.5*rnorm*cf(752)*dUdr(5)
c     &         - 0.5*rnorm*cf(753)*dUgr(1)
c     &         - 0.5*rnorm*cf(754)*dUgr(2)
c     &         - 0.5*rnorm*cf(755)*dUgr(3)
c     &         - 0.5*rnorm*cf(756)*dUgr(4)
c     &         - 0.5*rnorm*cf(757)*dUgr(5) 
     ...
     &     -0.5*rnorm*cf(767)*cf(315))
      ...
      dphidvp4(1,5)=cf(835)
      dphidvp5(1,5)=cf(836)
      dphidenormal(1,5)=cf(837)
      dphidenormal(2,5)=cf(838)
      dphidenormal(3,5)=cf(839)
      dphidevitno(1,5)=0.5*rnorm*cf(130)
      dphidug(1,5)=0.5*rnorm*(cf(69)-cf(776))-0.5*rnorm*cf(795)
      dphidug(2,5)=0.5*rnorm*(cf(70)-cf(777))-0.5*rnorm*cf(796)
      dphidug(3,5)=0.5*rnorm*(cf(71)-cf(778))-0.5*rnorm*cf(797)
      dphidug(4,5)=0.5*rnorm*(cf(72)-cf(779))-0.5*rnorm*cf(798)
      dphidug(5,5)=0.5*rnorm*(cf(132)-cf(780))-0.5*rnorm*cf(799)
      dphidug(6,5)=0.0d0
      dphidug(7,5)=0.0d0
      dphidud(1,5)=0.5*rnorm*(cf(123)-cf(781))-0.5*rnorm*cf(790)
      dphidud(2,5)=0.5*rnorm*(cf(124)-cf(782))-0.5*rnorm*cf(791)
      dphidud(3,5)=0.5*rnorm*(cf(125)-cf(783))-0.5*rnorm*cf(792)
      dphidud(4,5)=0.5*rnorm*(cf(126)-cf(784))-0.5*rnorm*cf(793)
      dphidud(5,5)=0.5*rnorm*(cf(133)-cf(785))-0.5*rnorm*cf(794)
      dphidud(6,5)=0.0d0
      dphidud(7,5)=0.0d0
      DO 90, I = 1,3
        dphidenormal(I,5) = dphidenormal(I,5) 
     &                    + dphidvp1(1,5)*dvp1denormal(I,1) 
     &                    + dphidvp4(1,5)*dvp4denormal(I,1) 
     &                    + dphidvp5(1,5)*dvp5denormal(I,1) 
   90 CONTINUE
      DO 100, I = 1,7
        dphidug(I,5) = dphidug(I,5) 
     &               + dphidvp1(1,5)*dvp1dug(I,1) 
     &               + dphidvp4(1,5)*dvp4dug(I,1) 
     &               + dphidvp5(1,5)*dvp5dug(I,1) 
        dphidud(I,5) = dphidud(I,5) 
     &               + dphidvp1(1,5)*dvp1dud(I,1) 
     &               + dphidvp4(1,5)*dvp4dud(I,1) 
     &               + dphidvp5(1,5)*dvp5dud(I,1) 
  100 CONTINUE
      dphidevitno(1,5) = dphidevitno(1,5) 
     &                 + dphidvp1(1,5)*dvp1devitno(1,1)
     &                 + dphidvp4(1,5)*dvp4devitno(1,1)
     &                 + dphidvp5(1,5)*dvp5devitno(1,1)
c      dphidudr(1,5)=-0.5*rnorm*cf(790)
c      dphidudr(2,5)=-0.5*rnorm*cf(791)
c      dphidudr(3,5)=-0.5*rnorm*cf(792)
c      dphidudr(4,5)=-0.5*rnorm*cf(793)
c      dphidudr(5,5)=-0.5*rnorm*cf(794)
c      dphidudr(6,5)=0.0d0
c      dphidudr(7,5)=0.0d0
c      dphidugr(1,5)=-0.5*rnorm*cf(795)
c      dphidugr(2,5)=-0.5*rnorm*cf(796)
c      dphidugr(3,5)=-0.5*rnorm*cf(797)
c      dphidugr(4,5)=-0.5*rnorm*cf(798)
c      dphidugr(5,5)=-0.5*rnorm*cf(799)
c      dphidugr(6,5)=0.0d0
c      dphidugr(7,5)=0.0d0
c      dphi(5) = cf(834)*dvp1
c     &        + cf(835)*dvp4
c     &        + cf(836)*dvp5
c     &        + cf(837)*denormal(1)
c     &        + cf(838)*denormal(2)
c     &        + cf(839)*denormal(3)
c     &        + 0.5*rnorm*cf(130)*devitno
c     &        + 0.5*rnorm*(cf(131)-cf(789))*dpstiff
c     &        + 0.5*rnorm*(cf(69)-cf(776))*dUg(1)
c     &        + 0.5*rnorm*(cf(70)-cf(777))*dUg(2)
c     &        + 0.5*rnorm*(cf(71)-cf(778))*dUg(3)
c     &        + 0.5*rnorm*(cf(72)-cf(779))*dUg(4)
c     &        + 0.5*rnorm*(cf(132)-cf(780))*dUg(5)
c     &        + 0.5*rnorm*(cf(123)-cf(781))*dUd(1)
c     &        + 0.5*rnorm*(cf(124)-cf(782))*dUd(2)
c     &        + 0.5*rnorm*(cf(125)-cf(783))*dUd(3)
c     &        + 0.5*rnorm*(cf(126)-cf(784))*dUd(4)
c     &        + 0.5*rnorm*(cf(133)-cf(785))*dUd(5)
c     &      - 0.5*rnorm*cf(790)*dUdr(1)
c     &      - 0.5*rnorm*cf(791)*dUdr(2)
c     &      - 0.5*rnorm*cf(792)*dUdr(3)
c     &      - 0.5*rnorm*cf(793)*dUdr(4)
c     &      - 0.5*rnorm*cf(794)*dUdr(5)
c     &      - 0.5*rnorm*cf(795)*dUgr(1)
c     &      - 0.5*rnorm*cf(796)*dUgr(2)
c     &      - 0.5*rnorm*cf(797)*dUgr(3)
c     &      - 0.5*rnorm*cf(798)*dUgr(4)
c     &      - 0.5*rnorm*cf(799)*dUgr(5)
      ...
      if (type.eq.1) then
        ...
        dphidvp1(1,6)=cf(840)*cf(802)
        dphidvp4(1,6)=cf(840)*cf(803) 
        dphidvp5(1,6)=cf(840)*cf(804)
        dphidenormal(1,6)=cf(840)*cf(805)
        dphidenormal(2,6)=cf(840)*cf(806)
        dphidenormal(3,6)=cf(840)*cf(807)
        dphidevitno(1,6)=cf(840)*cf(808)
        dphidug(1,6)=cf(840)*cf(810)+cf(840)*cf(825)
        dphidug(2,6)=cf(840)*cf(811)+cf(840)*cf(826)
        dphidug(3,6)=cf(840)*cf(812)+cf(840)*cf(827)
        dphidug(4,6)=cf(840)*cf(813)+cf(840)*cf(828)
        dphidug(5,6)=cf(840)*cf(814)+cf(840)*cf(829)
        dphidug(6,6)=0.0d0+phi(1)*updir
        dphidug(7,6)=0.0d0
        dphidud(1,6)=cf(840)*cf(815)+cf(840)*cf(820)
        dphidud(2,6)=cf(840)*cf(816)+cf(840)*cf(821)
        dphidud(3,6)=cf(840)*cf(817)+cf(840)*cf(822)
        dphidud(4,6)=cf(840)*cf(818)+cf(840)*cf(823)
        dphidud(5,6)=cf(840)*cf(819)+cf(840)*cf(824)
        dphidud(6,6)=0.0d0+phi(1)*(1.0d0-updir)
        dphidud(7,6)=0.0d0
      DO 110, I = 1,3
        dphidenormal(I,6) = dphidenormal(I,6) 
     &                    + dphidvp1(1,6)*dvp1denormal(I,1) 
     &                    + dphidvp4(1,6)*dvp4denormal(I,1) 
     &                    + dphidvp5(1,6)*dvp5denormal(I,1) 
  110 CONTINUE
      DO 120, I = 1,7
        dphidug(I,6) = dphidug(I,6) 
     &               + dphidvp1(1,6)*dvp1dug(I,1) 
     &               + dphidvp4(1,6)*dvp4dug(I,1) 
     &               + dphidvp5(1,6)*dvp5dug(I,1) 
        dphidud(I,6) = dphidud(I,6) 
     &               + dphidvp1(1,6)*dvp1dud(I,1) 
     &               + dphidvp4(1,6)*dvp4dud(I,1) 
     &               + dphidvp5(1,6)*dvp5dud(I,1) 
  120 CONTINUE
      dphidevitno(1,6) = dphidevitno(1,6) 
     &                 + dphidvp1(1,6)*dvp1devitno(1,1)
     &                 + dphidvp4(1,6)*dvp4devitno(1,1)
     &                 + dphidvp5(1,6)*dvp5devitno(1,1)
c        dphidudr(1,6)=cf(840)*cf(820)
c        dphidudr(2,6)=cf(840)*cf(821)
c        dphidudr(3,6)=cf(840)*cf(822)
c        dphidudr(4,6)=cf(840)*cf(823)
c        dphidudr(5,6)=cf(840)*cf(824)
c        dphidudr(6,6)=phi(1)*(1.0d0-updir)
c        dphidudr(7,6)=0.0d0
c        dphidugr(1,6)=cf(840)*cf(825)
c        dphidugr(2,6)=cf(840)*cf(826)
c        dphidugr(3,6)=cf(840)*cf(827)
c        dphidugr(4,6)=cf(840)*cf(828)
c        dphidugr(5,6)=cf(840)*cf(829)
c        dphidugr(6,6)=phi(1)*updir
c        dphidugr(7,6)=0.0d0
c        dphi(6) = cf(840)*cf(802)*dvp1
c     &         + cf(840)*cf(803)*dvp4
c     &         + cf(840)*cf(804)*dvp5
c     &         + cf(840)*cf(805)*denormal(1)
c     &         + cf(840)*cf(806)*denormal(2)
c     &         + cf(840)*cf(807)*denormal(3)
c     &         + cf(840)*cf(808)*devitno
c     &         + cf(840)*cf(809)*dpstiff
c     &         + cf(840)*cf(810)*dUg(1)
c     &         + cf(840)*cf(811)*dUg(2)
c     &         + cf(840)*cf(812)*dUg(3)
c     &         + cf(840)*cf(813)*dUg(4)
c     &         + cf(840)*cf(814)*dUg(5)
c     &         + cf(840)*cf(815)*dUd(1)
c     &         + cf(840)*cf(816)*dUd(2)
c     &         + cf(840)*cf(817)*dUd(3)
c     &         + cf(840)*cf(818)*dUd(4)
c     &         + cf(840)*cf(819)*dUd(5)
c     &         + cf(840)*cf(820)*dUdr(1)
c     &         + cf(840)*cf(821)*dUdr(2)
c     &         + cf(840)*cf(822)*dUdr(3)
c     &         + cf(840)*cf(823)*dUdr(4)
c     &         + cf(840)*cf(824)*dUdr(5)
c     &         + phi(1)*(1.0d0-updir)*dUdr(6)
c     &         + cf(840)*cf(825)*dUgr(1)
c     &         + cf(840)*cf(826)*dUgr(2)
c     &         + cf(840)*cf(827)*dUgr(3)
c     &         + cf(840)*cf(828)*dUgr(4)
c     &         + cf(840)*cf(829)*dUgr(5)
c     &         + phi(1)*updir*dUgr(6) 
      else if (type.eq.2) then
        updir = 0.5d0 + dsign(0.5d0, phi(1))
        phi(6) = phi(1) * (updir * Ugr(6) + (1.0d0 - updir) * Udr(6))
        cf(841)=(updir * Ugr(6) + (1.0d0 - updir) * Udr(6))
        dphidvp1(1,6)=cf(841)*cf(802)
        dphidvp4(1,6)=cf(841)*cf(803)
        dphidvp5(1,6)=cf(841)*cf(804)
        dphidenormal(1,6)=cf(841)*cf(805)
        dphidenormal(2,6)=cf(841)*cf(806)
        dphidenormal(3,6)=cf(841)*cf(807)
        dphidevitno(1,6)=cf(841)*cf(808)
        dphidug(1,6)=cf(841)*cf(810)+cf(841)*cf(825)
        dphidug(2,6)=cf(841)*cf(811)+cf(841)*cf(826)
        dphidug(3,6)=cf(841)*cf(812)+cf(841)*cf(827)
        dphidug(4,6)=cf(841)*cf(813)+cf(841)*cf(828)
        dphidug(5,6)=cf(841)*cf(814)+cf(841)*cf(829)
        dphidug(6,6)=0.0d0+phi(1)*updir
        dphidug(7,6)=0.0d0
        dphidud(1,6)=cf(841)*cf(815)+cf(841)*cf(820)
        dphidud(2,6)=cf(841)*cf(816)+cf(841)*cf(821)
        dphidud(3,6)=cf(841)*cf(817)+cf(841)*cf(822)
        dphidud(4,6)=cf(841)*cf(818)+cf(841)*cf(823)
        dphidud(5,6)=cf(841)*cf(819)+cf(841)*cf(824)
        dphidud(6,6)=0.0d0+phi(1)*(1.0d0-updir)
        dphidud(7,6)=0.0d0
      DO 130, I = 1,3
        dphidenormal(I,6) = dphidenormal(I,6) 
  ...
  140 CONTINUE
     ...
     &                 + dphidvp4(1,6)*dvp4devitno(1,1)
     &                 + dphidvp5(1,6)*dvp5devitno(1,1)
c        dphidudr(1,6)=cf(841)*cf(820)
c        dphidudr(2,6)=cf(841)*cf(821)
c        dphidudr(3,6)=cf(841)*cf(822)
c        dphidudr(4,6)=cf(841)*cf(823)
c        dphidudr(5,6)=cf(841)*cf(824)
c        dphidudr(6,6)=phi(1)*(1.0d0-updir)
c        dphidudr(7,6)=0.0d0
c        dphidugr(1,6)=cf(841)*cf(825)
c        dphidugr(2,6)=cf(841)*cf(826)
c        dphidugr(3,6)=cf(841)*cf(827)
c        dphidugr(4,6)=cf(841)*cf(828)
c        dphidugr(5,6)=cf(841)*cf(829)
c        dphidugr(6,6)=phi(1)*updir
c        dphidugr(7,6)=0.0d0
c        dphi(6) = cf(841)*cf(802)*dvp1
c     &         + cf(841)*cf(803)*dvp4
c     &         + cf(841)*cf(804)*dvp5
c     &         + cf(841)*cf(805)*denormal(1)
c     &         + cf(841)*cf(806)*denormal(2)
c     &         + cf(841)*cf(807)*denormal(3)
c     &         + cf(841)*cf(808)*devitno
c     &         + cf(841)*cf(809)*dpstiff
c     &         + cf(841)*cf(810)*dUg(1)
c     &         + cf(841)*cf(811)*dUg(2)
c     &         + cf(841)*cf(812)*dUg(3)
c     &         + cf(841)*cf(813)*dUg(4)
c     &         + cf(841)*cf(814)*dUg(5)
c     &         + cf(841)*cf(815)*dUd(1)
c     &         + cf(841)*cf(816)*dUd(2)
c     &         + cf(841)*cf(817)*dUd(3)
c     &         + cf(841)*cf(818)*dUd(4)
c     &         + cf(841)*cf(819)*dUd(5)
c     &         + cf(841)*cf(820)*dUdr(1)
c     &         + cf(841)*cf(821)*dUdr(2)
c     &         + cf(841)*cf(822)*dUdr(3)
c     &         + cf(841)*cf(823)*dUdr(4)
c     &         + cf(841)*cf(824)*dUdr(5)
c     &         + phi(1)*(1.0d0-updir)*dUdr(6)
c     &         + cf(841)*cf(825)*dUgr(1)
c     &         + cf(841)*cf(826)*dUgr(2)
c     &         + cf(841)*cf(827)*dUgr(3)
c     &         + cf(841)*cf(828)*dUgr(4)
c     &         + cf(841)*cf(829)*dUgr(5)
c     &          + phi(1)*updir*dUgr(6) 
        phi(7) = phi(1) * (updir * Ugr(7) + (1.0d0 - updir) * Udr(7))
        cf(842)=(updir * Ugr(7) + (1.0d0 - updir) * Udr(7))
        dphidvp1(1,7)=cf(842)*cf(802)
        dphidvp4(1,7)=cf(842)*cf(803)
        dphidvp5(1,7)=cf(842)*cf(804)
        dphidenormal(1,7)=cf(842)*cf(805)
        dphidenormal(2,7)=cf(842)*cf(806)
        dphidenormal(3,7)=cf(842)*cf(807)
        dphidevitno(1,7)=cf(842)*cf(808)
        dphidug(1,7)=cf(842)*cf(810)+cf(842)*cf(825)
        dphidug(2,7)=cf(842)*cf(811)+cf(842)*cf(826)
        dphidug(3,7)=cf(842)*cf(812)+cf(842)*cf(827)
        dphidug(4,7)=cf(842)*cf(813)+cf(842)*cf(828)
        dphidug(5,7)=cf(842)*cf(814)+cf(842)*cf(829)
        dphidug(6,7)=0.0d0
        dphidug(7,7)=0.0d0+phi(1)*updir
        dphidud(1,7)=cf(842)*cf(815)+cf(842)*cf(820)
        dphidud(2,7)=cf(842)*cf(816)+cf(842)*cf(821)
        dphidud(3,7)=cf(842)*cf(817)+cf(842)*cf(822)
        dphidud(4,7)=cf(842)*cf(818)+cf(842)*cf(823)
        dphidud(5,7)=cf(842)*cf(819)+cf(842)*cf(824)
        dphidud(6,7)=0.0d0
        dphidud(7,7)=0.0d0+phi(1)*(1.0d0-updir)
      DO 150, I = 1,3
        dphidenormal(I,7) = dphidenormal(I,7) 
     &                    + dphidvp1(1,7)*dvp1denormal(I,1) 
     &                    + dphidvp4(1,7)*dvp4denormal(I,1) 
     &                    + dphidvp5(1,7)*dvp5denormal(I,1) 
  150 CONTINUE
      DO 160, I = 1,7
        dphidug(I,7) = dphidug(I,7) 
     &               + dphidvp1(1,7)*dvp1dug(I,1) 
     &               + dphidvp4(1,7)*dvp4dug(I,1) 
     &               + dphidvp5(1,7)*dvp5dug(I,1) 
        dphidud(I,7) = dphidud(I,7) 
     &               + dphidvp1(1,7)*dvp1dud(I,1) 
     &               + dphidvp4(1,7)*dvp4dud(I,1) 
     &               + dphidvp5(1,7)*dvp5dud(I,1) 
  160 CONTINUE
      dphidevitno(1,7) = dphidevitno(1,7) 
     &                 + dphidvp1(1,7)*dvp1devitno(1,1)
     &                 + dphidvp4(1,7)*dvp4devitno(1,1)
     &                 + dphidvp5(1,7)*dvp5devitno(1,1)
c        dphidudr(1,7)=cf(842)*cf(820)
c        dphidudr(2,7)=cf(842)*cf(821)
c        dphidudr(3,7)=cf(842)*cf(822)
c        dphidudr(4,7)=cf(842)*cf(823)
c        dphidudr(5,7)=cf(842)*cf(824)
c        dphidudr(6,7)=0.0d0
c        dphidudr(7,7)=phi(1)*(1.0d0-updir)
c        dphidugr(1,7)=cf(842)*cf(825)
c        dphidugr(2,7)=cf(842)*cf(826)
c        dphidugr(3,7)=cf(842)*cf(827)
c        dphidugr(4,7)=cf(842)*cf(828)
c        dphidugr(5,7)=cf(842)*cf(829)
c        dphidugr(6,7)=0.0d0
c        dphidugr(7,7)=phi(1)*updir
c        dphi(7) = cf(842)*cf(802)*dvp1
c     &         + cf(842)*cf(803)*dvp4
c     &         + cf(842)*cf(804)*dvp5
c     &         + cf(842)*cf(805)*denormal(1)
c     &         + cf(842)*cf(806)*denormal(2)
c     &         + cf(842)*cf(807)*denormal(3)
c     &         + cf(842)*cf(808)*devitno
c     &         + cf(842)*cf(809)*dpstiff
c     &         + cf(842)*cf(810)*dUg(1)
c     &         + cf(842)*cf(811)*dUg(2)
c     &         + cf(842)*cf(812)*dUg(3)
c     &         + cf(842)*cf(813)*dUg(4)
c     &         + cf(842)*cf(814)*dUg(5)
c     &         + cf(842)*cf(815)*dUd(1)
c     &         + cf(842)*cf(816)*dUd(2)
c     &         + cf(842)*cf(817)*dUd(3)
c     &         + cf(842)*cf(818)*dUd(4)
c     &         + cf(842)*cf(819)*dUd(5)
c     &         + cf(842)*cf(820)*dUdr(1)
c     &         + cf(842)*cf(821)*dUdr(2)
c     &         + cf(842)*cf(822)*dUdr(3)
c     &         + cf(842)*cf(823)*dUdr(4)
c     &         + cf(842)*cf(824)*dUdr(5)
c     &         + phi(1)*(1.0d0-updir)*dUdr(7)
c     &         + cf(842)*cf(825)*dUgr(1)
c     &         + cf(842)*cf(826)*dUgr(2)
c     &         + cf(842)*cf(827)*dUgr(3)
c     &         + cf(842)*cf(828)*dUgr(4)
c     &         + cf(842)*cf(829)*dUgr(5)
c     &         + phi(1)*updir*dUgr(7) 
      endif
c
      END

      SUBROUTINE GENBCFLUXJWL(type,omega,A1,A2,R1r,R2r,enormal,evitno,
     &     U,Uinf,phi)
c-----------------------------------------------------------------------
...
c The EOS is P = omega*rhoe + f(rho)
c where f(rho) = A1*(1-omega*rho/R1r)*exp(-R1r/rho) 
c              + A2*(1-omega*rho/R2r)*exp(-R2r/rho) 
...
c-----------------------------------------------------------------------
      ...
      REAL*8 ooomega,omegap1,frho,frhoinf,fpinf
...
c
      ...
      frho =A1*(1.0-omega*U(1)/R1r)*exp(-R1r/U(1))
     &     +A2*(1.0-omega*U(1)/R2r)*exp(-R2r/U(1))
      frhoinf =A1*(1.0-omega*Uinf(1)/R1r)*exp(-R1r/Uinf(1))
     &        +A2*(1.0-omega*Uinf(1)/R2r)*exp(-R2r/Uinf(1))

      fpinf = A1*(-omega/R1r+(1.0-omega*Uinf(1)/R1r)*R1r/Uinf(1)**2)
     &          *exp(-R1r/Uinf(1))
     &      + A2*(-omega/R2r+(1.0-omega*Uinf(1)/R2r)*R2r/Uinf(1)**2)
     &          *exp(-R2r/Uinf(1))
...
c A few useful values for the computation
      ...
      enerinf  = 0.5d0*Vinf(1)*vitinf2 + (Vinf(5)-frhoinf)*ooomega
      ener     = 0.5d0*U(1)*vit2 + (U(5)-frho)*ooomega
...
c Again, a few useful values
      ...
      c2  = omegap1*Vinf(5)/Vinf(1) - frhoinf/Vinf(1) + fpinf
      c   = DSQRT(c2)
      c2  = 1.d0/c2
      q   = 0.5d0*vitinf2
...
c incoming domain
      ...
      flur1  = 
     &        ((normal(1)*(1.d0-omega*q*c2-fpinf*c2) - tet1)*temp(1) +
     &        (normal(1)*omega*Vinf(2)*c2)*temp(2)  +
     &        (normal(3)  + (normal(1)*omega*Vinf(3)*c2))*temp(3)   +
     &        (-normal(2) + (normal(1)*omega*Vinf(4)*c2))*temp(4)   -
     &        (normal(1)*omega*c2)*temp(5))

      flur2  =
     &        ((normal(2)*(1.d0-omega*q*c2-fpinf*c2) - tet2)*temp(1) +
     &        (-normal(3) + (normal(2)*omega*Vinf(2)*c2))*temp(2)   +
     &        (normal(2)*omega*Vinf(3)*c2)*temp(3)  +
     &        (normal(1)  + (normal(2)*omega*Vinf(4)*c2))*temp(4)   -
     &        (normal(2)*omega*c2)*temp(5))
     
      flur3  =
     &        ((normal(3)*(1.d0-omega*q*c2-fpinf*c2) - tet3)*temp(1) +
     &        (normal(2)  + (normal(3)*omega*Vinf(2)*c2))*temp(2)   +
     &        (-normal(1) + (normal(3)*omega*Vinf(3)*c2))*temp(3)   +
     &        (normal(3)*omega*Vinf(4)*c2)*temp(4)  -
     &        (normal(3)*omega*c2)*temp(5))
...
c incoming domain
      ...
      flur4   =
     &         ((-c*VdotNinf + omega*q + fpinf)*temp(1)  +
     &         ( c*normal(1) - omega*Vinf(2))*temp(2) +
     &         ( c*normal(2) - omega*Vinf(3))*temp(3) +
     &         ( c*normal(3) - omega*Vinf(4))*temp(4) +
     &         omega*temp(5))

      SUBROUTINE HLLEJAC(type,gamma,gam,pstiff,enormal,evitno,Ug,Ud,
     &                   jac,sw,mach,k1,cmach,shockreducer,irey,length,
     &                   prec)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      REAL*8 VdotN , VdotN1, VdotN2, vitno, rnorm, invnorm
      REAL*8 evitno
      REAL*8 g_VdotN(5)
...
c
      ...
      vp15 = 0.5d0*((1.d0+betaRT2)*VdotN1 - DSQRT(((1-betaRT2)*VdotN1)**2 +
     &       4.d0*betaRT2*cr_1**2))
c
      VdotN2 = normal(1)*Ud(2) + normal(2)*Ud(3) + normal(3)*Ud(4)
      cr_2 = DSQRT(gam*Ud(5)/Ud(1))
      vp24 = 0.5d0*((1+betaRT2)*VdotN2 + DSQRT(((1-betaRT2)*VdotN2)**2 +
     &       4.d0*betaRT2*cr_2**2))

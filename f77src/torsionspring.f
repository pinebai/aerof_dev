...
c         
         do cycle = 1, nucycles
c
            xi = coor(1,nu(map_pointer(face,cycle,2)))   
            yi = coor(2,nu(map_pointer(face,cycle,2)))
            zi = coor(3,nu(map_pointer(face,cycle,2)))
c
c
            xk = coor(1,nu(map_pointer(face,cycle,4))) 
            yk = coor(2,nu(map_pointer(face,cycle,4)))
            zk = coor(3,nu(map_pointer(face,cycle,4)))
c
c
c
c
...
c
      xj = coor(1,nu(map_pointer(face,cycle,1))) + 
     $     lambda*cvx(face,cycle)

      yj = coor(2,nu(map_pointer(face,cycle,1))) + 
     $     lambda*cvy(face,cycle)

      zj = coor(3,nu(map_pointer(face,cycle,1))) + 
...
c
      xqj  = xj - coor(1,nu(map_pointer(face,cycle,2)))
      yqj  = yj - coor(2,nu(map_pointer(face,cycle,2)))
      zqj  = zj - coor(3,nu(map_pointer(face,cycle,2)))
c 
